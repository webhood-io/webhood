...
function HeadersPopover({
  ...
  return (
    <Popover>
      ...
      <PopoverContent className="w-fit max-w-[90vw] truncate">
    ...
    <Table className="w-full table-fixed">
      ...
      <TableHeader>
        <TableRow>
          ...
          <TableHead className="w-[150px] md:w-[200px] lg:w-[450px] xl:w-[700px]">
            URL
          </TableHead>
        </TableRow>
      </TableHeader>
              ...
              <TableCell>
                {
                  <div className="flex flex-row gap-1">
                    ...
                    >
                      {Object.keys(value.request.headers).map((item, index) => {
                        return (
                          <DataItem
                            label={item}
                            content={value.request.headers[item]}
                            copy={true}
                          />
                        )
                      })}
                    ...
                    {value.response && (
                      ...
                      >
                        {Object.keys(value.response.headers).map(
                          (item, index) => {
                            return (
                              <DataItem
                                label={item}
                                content={value.response.headers[item]}
                                copy={true}
                              />
                            )
                          }
...
export default function Traceviewer({ scanItem }: { scanItem: ScansResponse }) {
  ...
  if (siteConfig.traceVersions.includes(traceObj.version)) {
    return <TraceTable traceData={traceObj} />
  } else {
    return (
      <div>
        Trace is older/newer version that is currently supported. Trace version
        is {traceObj.version || "unknown"}, supported versions are:{" "}
        {siteConfig.traceVersions.join(",")}
      </div>
    )
  }
}
