...
func CreateScannerToken(app core.App) *cobra.Command {
	return &cobra.Command{
		...
		Run: func(command *cobra.Command, args []string) {
			...
			fetchErr := error(nil)
			if scannerId == "" && username == "" {
				scannerId = "mzven27v6pg29mx" // default scanner id, this is auto generated by migrations (create_scanner_obj)
				authRecord, fetchErr = app.Dao().FindRecordById("api_tokens", scannerId)
			} else if username != "" {
				authRecord, fetchErr = app.Dao().FindAuthRecordByUsername("api_tokens", username)
			} else {
				authRecord, fetchErr = app.Dao().FindRecordById("api_tokens", scannerId)
			}
...
func CreateScanner(app core.App) *cobra.Command {
	return &cobra.Command{
		...
		Run: func(command *cobra.Command, args []string) {
			...
			println("New scanner created with id: "+apiTokenRecord.Id, "and config id: "+configRecord.Id, "and username: "+username)
		},
	}
}
